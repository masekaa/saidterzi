...
Dual momentum'u **on bir varlık evreninde** canlı hesaplayan dashboard:
...
|--------|---------|----------|
| **Yahoo Finance** (v8 chart) | Yok | Fiyat/total-return serileri (11 evren: ETF, hisse, kripto, sektör, uluslararası, emtia, faktör, tahvil, varlık-sınıfı, ülke, BIST 100 — sonuncu USD bazlı, USDTRY=X ile çevrilir) — aylık normalize |
...
| `lib/` | `yahoo.ts` (veri çekme + aylık normalizasyon) · `calc.ts` (formül-belgeli finansal primitifler) · `universe.ts` (11 evren + parametreler) · `fx.ts` (TRY→USD çevrimi, BIST için) · `methods.ts` (şeffaf yöntem hesaplayıcıları + evren-bağımsız pano/momentum üreticileri) · `backtest.ts` (momentum rotasyon simülasyonu + işlem maliyeti) · `factors.ts` (Fama-French OLS alpha) · `fundamentals.ts` (FMP earnings) · `concurrency.ts` (eşzamanlılık-sınırlı fetch, rate-limit koruması) · `zip.ts` (bağımlılıksız ZIP okuyucu) · `types.ts` |

### Dashboard ne gösterir

**Genel bakış (en üst):**
- **📋 Bu Ay Ne Yapmalı? — Sade Özet:** sayfanın en üstünde, jargonsuz **yatırım karar-destek motoru** ("Ayşe teyzeye anlatır gibi"). Tek-bakış **Genel Durum verdikti** (Risk Açık / Karışık / Savunmada — birden çok sinyalin sentezi) + madde madde: bu ayın ana kararı (piyasada kal / nakitte bekle — **güvenlik tamponu** & **pozisyon oynaklığı** ile nicel), 🇹🇷 **BIST 100** özel vurgusu (tampon + oynaklık), sinyal kararlılığı (whipsaw), piyasa genişliği, geçmiş 1-yıl tutma ihtimalleri, güncel drawdown, **oynaklık rejimi**, **10-ay MA trend** (Faber), 60/40 düşüş kıyası, **12-1 momentum kırılganlık** uyarısı. Çekirdek + uyarılar her zaman görünür, ayrıntı katlanır; ay damgalı; **Kopyala** (düz metin) + yazdır/PDF ile aylık rapor.
- **⚡ Öne Çıkanlar:** otomatik içgörüler — en yüksek Sharpe stratejisi, bileşik büyümesi, savunma duruşu, en iyi çeşitlendirici, **piyasa genişliği** (pozitif momentumlu varlık oranı = risk-on/off), **istatistiksel güven** (en iyi stratejinin Olasılıksal Sharpe'ı — kenar şans eseri mi?).
- **🗺️ Evren Momentum Gücü:** evrenler-arası risk-iştahı haritası — her evrende T-Bill eşiğini geçen varlık oranı, güçlüden zayıfa sıralı barlar (bu ay nerede risk-on?).
- **Bu Ayın Sinyalleri:** 11 evrenin güncel pozisyonları tek bakışta + **sinyal kırılganlığı** (T-Bill eşiğine pay, ince payda nakde dönme uyarısı).
...
- **Strateji Karşılaştırma:** tüm stratejiler **sıralanabilir** tabloda (CAGR/Sharpe/Sortino/MaxDD/getiri/devir) + **PSR sütunu** (Olasılıksal Sharpe = Sharpe'ın gerçekte >0 olma olasılığı, renk-kodlu) + **Deflated Sharpe** (çoklu-deneme/seçim-yanlılığı düzeltmesi, Bailey–López de Prado); "Momentum Al-Tut'u Yeniyor mu?" tablosu — CAGR/Sharpe farkı + **Bilgi Oranı (IR)** + **aylık fark t-stat (istatistiksel anlamlılık)**; **ortak-dönem overlay** + **evrenler-arası risk–getiri dağılımı** (adil kıyas, çeşitlendirme görsel kanıtı).
- **📊 Momentum Dağılımı:** evrenler-arası kesitsel getiri standart sapması — relative momentum şu an hangi evrende ne kadar ayırt edici (yüksek dağılım = güçlü lider-takipçi farkı, momentum tarihsel olarak daha etkili).
...
- **🧩 Dual Momentum Bileşik:** 11 evrenin **eşit-ağırlık + risk-parity** meta-stratejisi — güncel duruş, **"bu ay al" listesi (CSV)**, **pasif al-tut benchmark karşılaştırması**, **Bileşik Faktör Alpha** (Fama-French 3 — meta-strateji piyasa/boyut/değer beta ötesinde alpha üretiyor mu?), **⚖️ 60/40 (SPY/AGG) evrensel kıyas** (stat tablosu + log büyüme overlay + risk-getiri/ortak-dönem grafiklerinde referans), getiri atfı, risk-parity ağırlıkları, **Çeşitlendirme Oranı** (Choueifaty–Coignard — DR + etkin bağımsız bahis sayısı), korelasyon matrisi + **24-ay rolling ortalama korelasyon**, **🧮 Marjinal Sleeve Katkısı** (leave-one-out — her evren değer katıyor mu?), drawdown epizodları, **kriz stres testi** ve **blok-bootstrap risk dağılımı**.
- **🧮 Marjinal Sleeve Katkısı & 📅 Yıllık Getiri Matrisi:** (katlanabilir) leave-one-out marjinal katkı + stratejiler × takvim-yılları ısı tablosu (liderlik rotasyonu, CSV).

**Her evren sekmesinde (ETF/Hisse/Kripto/Sektör/Uluslararası/Emtia/Faktör/Tahvil/Varlık Sınıfı/Ülke/BIST 100):**
- **Headline stat şeridi** (CAGR/Sharpe/Sortino/MaxDD) + **sinyal panosu** (sıralanabilir, **yol-kalitesi** sütunu = trailing 12-ay % pozitif ay, Gray–Vogel 2016; **12-1 momentum** sütunu = son ayı atlayan getiri, Jegadeesh-Titman — 12-ay ile zıt işaretliyse ⚠ tersine dönüş riski; **yıllık oynaklık** sütunu = trailing 12-ay yıllıklandırılmış σ, risk bağlamı; **risk-ayarlı lider notu** = top-N sepetinde getiri÷oynaklık lideri ham getiri liderinden farklıysa uyarı) + momentum sıralaması + **look-back duyarlılık matrisi** + **göreli güç** (kümülatif strateji÷benchmark).
- **Görsel analiz** (katlanabilir, paylaşılan `BacktestCharts`): equity curve (log) · pozisyon bandı · **işlem günlüğü** (pozisyon tutuş dönemleri + işlem-bazlı kazanma oranı/payoff/en iyi-kötü işlem, CSV) · drawdown (underwater) + **en kötü 5 epizod** · **karşılaştırmalı underwater** (strateji vs benchmark drawdown overlay — düşüş-koruma tezi) · aylık ısı haritası · **takvim-yılı getirileri (strateji vs benchmark)** · **kriz stres testi** (2008/2020/2022 vb.) · **blok-bootstrap risk dağılımı** · **yarı-dönem tutarlılık** (örneklem-dışı kenar-kalıcılığı) · **giriş-tarihi duyarlılığı** (lump-sum nihai CAGR'ın giriş yılına göre aralığı — timing/sıra riski) · **kazanç/kayıp serileri** (sequence risk) · **Olasılıksal Sharpe (PSR)** + MinTRL (Bailey–López de Prado) · **vol-hedefli versiyon** (oynaklık-yönetimli momentum, Barroso–Santa-Clara) · risk–getiri · 12-ay rolling getiri & **volatilite** & **Sharpe** & **göreli performans (+ vuruş ortalaması)** · getiri scatter · **yukarı/aşağı yakalama** · box plot · **getiri histogramı (normal eğri bindirmeli)** · **mevsimsellik**.
...
- **Fama-French faktör alpha** (alpha, market/size/value beta, R²) + **şeffaf yöntem kartları** (trailing, relative, absolute, MA, trend-line t-stat, 52-hafta, hızlanan, frog-in-the-pan (bilgi sürekliliği), taze/bayat, trend salience, risk parity; ETF'de ayrıca GBM, DMSR, GEM).
