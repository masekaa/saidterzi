...
- **Veri tazeliği & önbellek:** `/api/analysis` ~95 sembol (11 evren + çekirdek; BIST USD'ye çevirme için USDTRY=X dâhil) + Ken French + (varsa) FMP çeker; sonuç sunucu-içi **10 dk önbellekte** tutulur (Yahoo rate-limit + hız). "Yenile" butonu `?refresh=1` ile önbelleği atlar. Fetch'ler **eşzamanlılık-sınırlı** (`lib/concurrency.ts`, en çok 12 paralel istek) — Yahoo 429 riskini azaltır. Fonksiyon süresi `maxDuration=60` ile uzatıldı.
...
- **Yahoo güvenilirliği:** Resmi olmayan endpoint; nadiren 403/değişiklik olabilir. UI hata durumunda "Yenile" ile tekrar dener; ayrıca `/api/analysis` başarısız sembolleri bir kez yeniden çeker ve her istek 10 sn timeout'ludur (asılı istek tüm analizi bloklamaz). **Not (2026-06):** İkinci kaynak adayı Stooq artık CSV yerine JavaScript proof-of-work anti-bot ekranı döndürüyor → sunucu-taraflı (Vercel) çekim için uygun değil. Alpha Vantage anahtar ister (ücretsiz katman 25 istek/gün — ~95 sembollük tek yüklemeye yetmez). Pratik dayanıklılık şu an: retry + per-istek timeout + 10 dk önbellek + sembol-bazlı kapsama uyarıları.
